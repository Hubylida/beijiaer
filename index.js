var nav = document.querySelectorAll('.nav-bar');
nav[0].style.borderTopColor = "#000";
// ................
...
[
   ...
   for(let i = 0; i < pictureL; i++){
        hoverPic[i].addEventListener('mouseover',function(){
                hoverPic[i].src = hoverPicSrc[i];           
        });
        hoverPic[i].addEventListener('mouseout',function(){
            hoverPic[i].src = hoverPicSrcF[i];
        });
    }
...
function scrollBack(){
    ...
    if(currentScroll != 0){
        ...
        timer = setTimeout('scrollBack()',10)
    }else {
        clearTimeout(timer)
    }
}
